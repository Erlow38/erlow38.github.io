...
const Contact = () => {
  ...
  const socialLinks = [
    ...
    { icon: <MoreHorizontal size={24} />, href: 'https://erlinks.github.io/', labelKey: 'moreNetworks' as const, color: 'from-orange-500 to-red-500' },
  ]
  ...
  return (
    <section id="contact" className="py-20 md:py-32 relative overflow-hidden">
      <div className="container mx-auto px-4 md:px-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 max-w-6xl mx-auto">
          ...
          >
                ...
                >
                  ...
                  <div className="relative flex items-center gap-4">
                    ...
                    <div className="flex-1">
                      <h4 className="text-xs text-gray-400 mb-1 uppercase tracking-wide">{t(info.titleKey)}</h4>
                      {info.link ? (
                        <a
                          href={info.link}
                        ...
                        <p className="text-white font-semibold text-sm md:text-base">{'valueKey' in info ? t(info.valueKey) : ''}</p>
                      )}
                    </div>
                  </div>
                </motion.div>
            ...
            <div className="glass-effect p-6 rounded-3xl border-2 border-white/10">
              <h4 className="text-white font-black mb-4 text-lg">{t('socialNetworks')}</h4>
              <div className="flex gap-3">
                {socialLinks.map((social, index) => (
                  <motion.a
                    key={'labelKey' in social ? social.labelKey : social.label}
                    href={social.href}
                    target="_blank"
                    rel="noopener noreferrer"
                    aria-label={'labelKey' in social ? t(social.labelKey as 'moreNetworks') : social.label}
          ...
          >
            <form onSubmit={handleSubmit} className="glass-effect p-6 md:p-8 rounded-3xl space-y-5 border-2 border-purple-500/20 relative overflow-hidden">
              ...
              <div className="relative">
                <h3 className="text-2xl font-black text-white mb-6">{t('sendMessage')}</h3>
        ...
        >
          <p className="text-gray-400 text-sm md:text-base">
            {t('footer')} <span className="text-purple-400">Ethan Ehrler</span>
          </p>
        </motion.div>
      </div>
    </section>
